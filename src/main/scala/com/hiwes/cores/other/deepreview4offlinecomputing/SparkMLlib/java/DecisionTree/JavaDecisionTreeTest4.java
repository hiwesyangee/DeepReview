...
/**
 * 本次要求如下，包含步骤进行分类：
 * 1.将行车记录数据进行筛选(	找出19-21点之间的所有数据，按照经纬度积累成20个类别。
 * 意思是说——用filter筛选出符合要求是的数据，并定义k = 20；使用聚类算法K-Means，得到结果1——————
 * 建立模型，并得到20个中心点。假定此时的数据还有3万条		)
 * 此时做一步操作，将每一条数据来自哪一类做出判断，并在前面(或后面)加上字符串
 * 也可以把属于哪一类作为K，把经纬度设为V，这样存在RDD里面也可以。
 * 2.假定此时的数据有3万条，那么拿出500条数据作为测试数据（这五百条作为测试数据，只需要经纬度，不需要知道它们属于哪一个类）。剩下的29500条数据用来建模和测试（70%建模，30%测试），用决策树算法得到结果2——————决策树模型。用30%取验证准确率有多高。算正确率多高	得到一个百分比。
 * 3.将刚刚的500条数据再进行测试，与刚刚得到的结果进行比对，得到一个结果3——————符合要求的百分比
 * 4.打印出百分比
 */

public class JavaDecisionTreeTest4 implements Serializable {
    public static void main(String[] args) throws Exception {
        ...
        JavaRDD<Vector> parsedData3 = parsedData2.map(s -> {
        ...
        FileOutputStream fos = new FileOutputStream("file:///Users/hiwes/data/mllib/test2.txt");
        while (input.hasNext()) {
            String a = input.next();
            fos.write((a + "\r\n").getBytes());
        }
