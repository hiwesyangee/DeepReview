...
public class HBaseUtils {
    ...
    public static boolean createTable(String tableName, String[] cfs) {
        try (HBaseAdmin admin = HBaseUtils.getInstance().admin) {
    ...
    public static boolean putRow(String tableName, String rowKey, String cfName, String qualifier, String data) {
        try (Table table = HBaseUtils.getInstance().getTable(tableName)) {
    ...
    public static void main(String[] args) {
        ...
        String column = "id";
        // String rowkey = "1";
        HBaseUtils.getInstance().createTable("t1", cfs);

        long start = System.currentTimeMillis();
        System.out.println("开始第一次写入数据: ");
        /** 此处接入100k条数据，直接进行批处理进行操作 */
        for (int i = 0; i < 100000; i++) {
            HBaseUtils.getInstance().bulkput(tableName, i + "", cf, column, String.valueOf(100321 + i));
        }
        ...
        for (int i = 0; i < 100000; i++) {
            HBaseUtils.getValue(tableName, i + "", cf, column);
        }
        ...
        HBaseUtils.getScanner(tableName, "0", "999999");
        ...
        new Thread(new Runnable() {
            public void run() {
                HBaseUtils.getScanner(tableName, "1000000", "1999999");
            }
        }).start();
